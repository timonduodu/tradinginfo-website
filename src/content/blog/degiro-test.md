---
title: "DEGIRO im Test: Die günstige Alternative für Schweizer Anleger"
description: "DEGIRO ist einer der günstigsten Broker für ETF-Sparpläne und einfache Aktien-Investments. Ein detaillierter Test der Stärken und Schwächen für Schweizer Kunden."
pubDate: 2026-04-20
category: "Broker"
subcategory: "Tests"
author: "Redaktion"
readingTime: "9 Min. Lesezeit"
---

DEGIRO ist seit Jahren der Geheimtipp für kostenbewusste europäische Anleger. Mit niedrigen Kommissionen, einer kernfreien ETF-Liste und einer einfachen Plattform hat sich der niederländische Broker als ernsthafte Alternative zu klassischen Anbietern etabliert. Für Schweizer Anleger lohnt ein genauer Blick.

## Hintergrund und Regulierung

DEGIRO wurde 2008 in Amsterdam gegründet und gehört seit 2020 zur deutschen flatexDEGIRO Bank AG. Die regulatorische Aufsicht erfolgt durch die deutsche BaFin und die niederländische AFM. Schweizer Kunden werden über die deutsche Einheit bedient.

Wichtig: DEGIRO ist eine Bank, was Vor- und Nachteile mit sich bringt. Vorteilhaft ist die Einlagensicherung (bis 100'000 EUR über die deutsche Einlagensicherung). Nachteilhaft sind teilweise restriktive Compliance-Vorgaben, etwa bei der Erweiterung der Konten um Margin-Funktionen.

> *"Cheap is not the same as cheapest in the long run. But for index investing, cheap is often enough."*
>, **John Bogle**

## Gebührenstruktur

DEGIRO arbeitet mit einer **transaktionsbasierten Gebührenstruktur** ohne Depotgebühren oder Inaktivitätskosten. Die wichtigsten Kosten:

### Aktien

| Markt | Kommission pro Trade |
|---|---|
| Schweiz (SIX) | 4.90 CHF + 0.05 % vom Volumen |
| USA (NYSE, NASDAQ) | 2.00 USD + 0.005 % |
| Deutschland (Xetra) | 4.90 EUR + 0.05 % |
| UK (LSE) | 4.90 GBP + 0.05 % |

Eine 5'000-CHF-Position in einer Schweizer Aktie kostet also 4.90 CHF + 2.50 CHF = **7.40 CHF Kommission**, plus 3.75 CHF Stempelabgabe = **11.15 CHF Gesamtkosten**.

### ETFs, der grosse Vorteil

DEGIRO bietet eine **"Kernselektion"** von rund 200 ETFs, die kostenlos handelbar sind. Diese Liste enthält die wichtigsten Welt-, USA-, Europa- und Themenindex-ETFs.

Bedingungen für die kostenlose Ausführung:
- Mindestens einmal pro Monat im jeweiligen Produkt traden
- Bei Trades über 1'000 EUR Wert: keine Kommission
- Bei Trades unter 1'000 EUR: nur einmal pro Monat kostenlos, danach 2 EUR

Diese Konstruktion macht DEGIRO ideal für **monatliche ETF-Sparpläne**. Ein Anleger, der monatlich für 500 bis 5'000 CHF einen Welt-ETF kauft, zahlt praktisch keine Gebühren, abgesehen von der Stempelabgabe.

### Devisenwechsel

DEGIRO bietet zwei FX-Modi an:

**AutoFX (Standard):** Bei Trades in Fremdwährung wird automatisch zum aktuellen Marktkurs plus 0.25 % gewechselt.

**Manual FX:** Aktiver Devisenwechsel auf Anfrage zu 0.10 % zuzüglich 10 EUR Gebühr.

Für regelmässige USD-Trades ist Manual FX bei höheren Beträgen günstiger, bei kleineren AutoFX einfacher.

## Plattform und Bedienung

Die DEGIRO-Plattform ist bewusst minimalistisch. Die Web-Oberfläche bietet:

- Einfache Order-Eingabe (Market, Limit, Stop, Stop-Limit)
- Basic Chart-Funktionen
- Portfolio-Übersicht
- Order-Historie und Steuerreports
- Watchlists

Die mobile App ist funktional, aber nicht ausgereift. Wer professionelle Charting-Tools, fortgeschrittene Order-Typen oder Algo-Trading erwartet, ist bei DEGIRO falsch.

Für die Zielgruppe, Privatanleger mit einfachen ETF- und Aktien-Strategien, ist die Plattform jedoch ausreichend. Die Reduktion auf das Wesentliche ist kein Nachteil, sondern Teil des Konzepts.

## Verfügbare Märkte

DEGIRO bietet Zugang zu rund 50 Börsenplätzen weltweit, darunter alle wichtigen europäischen, US- und asiatischen Märkte. Was fehlt:

- Direkter Handel an exotischen Märkten (Brasilien, Indien, Ägypten etc.)
- Forex-Spot-Markt
- Derivate-Märkte ausser Optionen auf wichtige Aktien und Indizes
- Bond-Sekundärmarkt (sehr eingeschränkt)
- Strukturierte Produkte aus der Schweiz

Für klassisches Aktien- und ETF-Investing ist die Marktbreite mehr als ausreichend. Wer Spezialprodukte handelt, braucht einen anderen Broker.

## Sicherheit und Risiken

DEGIRO trennt Kundenvermögen vom Eigenkapital. Aktien werden in einem separaten Fund (SPV) gehalten, Cash über deutsche Banken (DEGIRO Geldmarktfonds oder direkt bei flatexDEGIRO). Die Konstruktion ist üblich und sicher.

Im historischen Kontext: DEGIRO hatte 2020 eine Phase regulatorischer Probleme in den Niederlanden (AFM-Sanktionen wegen Compliance-Mängeln). Diese sind inzwischen behoben, hinterliessen aber Spuren im öffentlichen Image. Operative Auswirkungen auf Kunden gab es keine.

> *"Regulatory issues are a normal part of every brokerage's history. What matters is how they handle them, not whether they have them."*
>, **Compliance-Officer eines schweizer Asset Managers**

## DEGIRO für Schweizer Anleger: Pros

**1. Sehr günstige Kosten** für ETF-Sparpläne und einfache Aktien-Investments.

**2. Stempelabgabe entfällt**, DEGIRO ist kein Schweizer Effektenhändler, daher keine 0.075/0.15 % Umsatzabgabe.

**3. Einfache Bedienung** ideal für Einsteiger und passive Anleger.

**4. Gute Auswahl an Schweizer und europäischen Aktien.**

**5. Transparente Gebührenstruktur** ohne versteckte Inaktivitätskosten oder Depotgebühren.

## DEGIRO für Schweizer Anleger: Contras

**1. Steuerauszug nicht Schweizer-konform**, die Schweizer Steuererklärung erfordert manuelle Aufarbeitung der DA-1-Liste.

**2. Mangelhafter Kundenservice in Deutsch**, primär englisch- und niederländischsprachig, längere Antwortzeiten.

**3. Limitierte Order-Typen**, keine Conditional Orders, kein OCO, kein Trailing-Stop.

**4. Kein Krypto-Direkthandel.**

**5. Margin-Konto nicht standardmässig**, muss explizit beantragt werden, oft mit Verzögerungen.

**6. Veraltete Chart-Funktionen** im Vergleich zu modernen Plattformen.

## Wer DEGIRO nutzen sollte

DEGIRO ist die richtige Wahl für:

- ETF-Sparpläne mit monatlichen Investments unter 5'000 CHF
- Buy-and-Hold-Anleger mit langem Anlagehorizont
- Studenten und junge Erwachsene, die mit kleinen Beträgen starten
- Nebendepots als Ergänzung zu einem Hauptbroker
- Kostenbewusste Anleger ohne Bedarf an Spezialprodukten

DEGIRO ist **nicht** die richtige Wahl für:

- Aktive Trader mit hoher Trade-Frequenz
- Derivate-Strategien (Optionen, Futures)
- Komplexe Steuersituationen
- Vermögensverwaltung mit Premium-Service-Anspruch
- Internationale Anleger mit hohem Bedarf an Diversifikation in exotische Märkte

## Vergleich zu Konkurrenten

| Aspekt | DEGIRO | Yuh | Interactive Brokers | Swissquote |
|---|---|---|---|---|
| ETF-Sparplan | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐ |
| Aktive Trader | ⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ |
| Schweizer Steuerauszug | ⭐ | ⭐⭐⭐⭐⭐ | ⭐ | ⭐⭐⭐⭐⭐ |
| Kostenniveau | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ |
| Marktbreite | ⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
| Bedienkomfort | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐⭐⭐ |

## Fazit

DEGIRO ist die rationale Wahl für eine sehr spezifische Zielgruppe: Schweizer Anleger, die einfache ETF- und Aktien-Strategien zu minimalen Kosten umsetzen wollen, ohne Bedarf an Premium-Service oder Spezialprodukten zu haben.

Wer diese Anforderungen erfüllt, spart über die Jahre substantielle Beträge gegenüber teureren Anbietern wie Swissquote, bei vergleichbarer Funktionalität für die Kernaufgabe. Für aktive Trader oder Anleger mit höheren Ansprüchen ist DEGIRO hingegen suboptimal.

Ein bewährtes Modell für Schweizer Anleger: DEGIRO als Hauptdepot für ETF-Sparplan, Swissquote als Schweizer Zweitdepot für direkte SIX-Aktien, Interactive Brokers für komplexere Strategien, jeder Broker für das, was er am besten kann.

---

*Dieser Artikel basiert auf öffentlich verfügbaren Konditionen Stand 2026 und ist sachlich neutral verfasst.*
